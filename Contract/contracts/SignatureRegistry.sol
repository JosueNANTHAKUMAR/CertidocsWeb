...
contract SignatureRegistry {
    ...
    function storeSignature(
        ...
        bytes32 signatureId = keccak256(
            abi.encodePacked(msg.sender, _messageHash, block.timestamp)
        );
    ...
    function verifySignature(
        ...
        address recoveredSigner = recoverSigner(_messageHash, data.signature);
        ...
        if (!isAuthorized) {
            return (false, data.signer, recoveredSigner, data.messageHash, _messageHash);
        }

        bool isValid = (data.signer == recoveredSigner) &&
                    (data.messageHash == _messageHash) &&
                    (block.timestamp <= data.expiration);

        return (isValid, data.signer, recoveredSigner, data.messageHash, _messageHash);
    }
